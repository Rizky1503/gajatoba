...
// 			$query = "SELECT * FROM aluni_pengguna a INNER JOIN aluni_pengguna_hak_akses b ON a.`username` = b.`username` WHERE a.`username` = '$username' AND a.`password` = '$password_salted' AND a.`aktif` = 'ya'";
			...
			$query = "SELECT * FROM aluni_pengguna a INNER JOIN aluni_pengguna_hak_akses b ON a.`username` = b.`username` WHERE a.`username` = 'admin' AND a.`password` = '673890cbfcd78f49a69e1407173abf1e5323c54f06038534bf21fecb64149ba12791a7beccbbf85b6a984a8907f83599e8d8b27e6b22338945a8526316b813bf' AND a.`aktif` = 'ya'";
	...
	*/
	elseif ($aksi == "tambah_anggota") {
		...
		if ($sukses_upload) {
			// Simpan Dasar
			$query      = "INSERT INTO aluni_anggota_dasar (nama_lengkap, nama_panggilan, jenis_kelamin, tempat_lahir, tanggal_lahir, agama, foto, id_provinsi, id_kota, alamat, aktif, created_by, created_date, updated_by, updated_date, revisi) VALUES ('$nama_lengkap', '$nama_panggilan', '$jenis_kelamin', '$tempat_lahir', '$tanggal_lahir', '$agama', '$foto', '$id_provinsi', '$id_kota', '$alamat', '$aktif', '$_SESSION[a2_username]', NOW(), '$_SESSION[a2_username]', NOW(), '0')";
